const fs = require('fs');
const { app, dialog, shell, ipcRenderer } = require('electron');
...
if (!fs.existsSync(modsPath)) {
  ...
  console.log(`Folder "${modsPath}" created.`);
}
  ...
  fetch('main.html')
    ...
    .then(data => {
      ...
      fileInput.addEventListener('change', async () => {
        ...
        const validExtensions = ['.jar', '.zip', '.rar'];
        ...
        try {
          ...
          if (fileExtension === '.zip') {
            ...
            await fs.createReadStream(targetPath)
              .pipe(unzipper.Extract({ path: modsPath }))
              .promise();
            fuzeLauncher.log('Zip file extracted');

            // Delete the original zip file
            fs.unlinkSync(targetPath);
            fuzeLauncher.log('Original zip file deleted');
            document.getElementById('mods-subtitle').textContent = "Ajoute des mods si l'event en a besoin"
          }

          if (fileExtension === '.rar') {
            document.getElementById('mods-subtitle').textContent = "EXTRACTION(S) RAR EN COURS..."
            console.log(targetPath, modsDirectory)
            fuzeLauncher.extractRAR2(targetPath, modsDirectory);
          }

          // You can use IPC to notify the main process if needed
          ipcRenderer.send('file-moved', targetPath);
        } catch (error) {
...
document.addEventListener('paste', function (event) {
  ...
  select(eventData.eventType.toLowerCase());
...
async function launchGame() {
  ...
  launchButton.textContent = "CHARGEMENT DES LIBRARIES..."
  const { Client, Authenticator } = require("minecraft-launcher-core");
  const launcher = new Client();
  //Import the Auth class
  const { Auth } = require("msmc");

  launchButton.textContent = "CHARGEMENT DES VERSIONS..."
  ...
  if (settingsContent.account.type === "microsoft") {
    launchButton.textContent = "SELECTE UN COMPTE"
  ...
  if (document.querySelector('.item-selected').id === 'vanilla') {
    ...
    if (settingsContent.account.type === 'crack') {
      ...
      launchButton.textContent = "DÉMARRAGE..."
      fuzeLauncher.log("Starting!");
      launcher.launch(opts);
      launchButton.textContent = "JEU OUVERT"
      ...
      if (settingsContent.account.type === 'crack') {
        ...
        authManager.launch("raw").then(async xboxManager => {
          ...
          let opts = {
            ...
            //forge: `./.minecraft/forge/Forge_${version}.jar`,
            forge: `./.minecraft/forgeJars/${forgeFileName}`,
            memory: {
              max: maxram,
              min: minram
